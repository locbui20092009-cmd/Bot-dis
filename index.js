...
client.on('interactionCreate', async i => {
        ...
        setImmediate(async () => {
            try {
                ...
                const [resS, resSM, resOCT] = await Promise.allSettled([
                    ...
                    remOCT > 0 ? axios.get(`https://octolink.vip/api?api=${OCTOLINKZ_API_KEY}&url=${encodeURIComponent(targetOCT)}&type=1`, axiosConfig) : null
                ]);
    ...
    if (i.commandName === 'doithuong') {
        setImmediate(async () => {
            ...
            try {
                const ch = await client.channels.fetch(ADMIN_CHANNEL_ID);
                if (ch) ch.send({ embeds: [new EmbedBuilder().setTitle('🎁 YÊU CẦU ĐỔI THƯỜNG').addFields({name:'Người đổi',value:`<@${id}>`},{name:'Tên quà',value:q},{name:'Xu trừ',value:`-${g} xu`}).setTimestamp()] });
            } catch {}
        });
    }
